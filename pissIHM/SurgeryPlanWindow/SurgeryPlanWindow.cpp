...
void SurgeryPlanWindow::display(vtkImageData *imgToBeDisplayed){
    ...
    vesselPoints =new QVector<CenterLinePoint*>();
    filename = new QFile("E:\\dataset04\\reference0-4.txt");
    if(!filename->open(QIODevice::ReadOnly|QIODevice::Text  )){
        qDebug()<<"can't find the reference file!";
    }
    int pointsCount = 1;
    while(!filename->atEnd()){
        ...
        p->set_sagittal(parame[2].toDouble(0));
        vesselPoints->append(p);
        pointsCount ++;
    }

    for(int i=0; i<vesselPoints->size();i++){
        points->InsertPoint(i,
                            vesselPoints->at(i)->get_abscissa(),
                            vesselPoints->at(i)->get_vertical(),
                            vesselPoints->at(i)->get_sagittal());

    }
    poly->GetPointIds()->SetNumberOfIds(vesselPoints->size());
    ...
    mapperreferencePath->SetInputData(grid);
    actorreferencePath->SetMapper(mapperreferencePath);
    actorreferencePath->GetProperty()->SetColor(0,0,0);
    actorreferencePath->GetProperty()->SetOpacity(0.3);
    actorreferencePath->GetProperty()->SetPointSize(0.89);
